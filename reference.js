...
async function handleRequest(request) {
	...
	try {
		...
		return getHTMLResponse(
			'Something is wrong while trying to get your cookie: <br> siteCookie: ' + siteCookie + '<br>' + 'lastSite: ' + lastVisit
		);
	}
	...
	for (var pair of request.headers.entries()) {
		...
		clientHeaderWithChange.set(
			pair[0],
			pair[1].replaceAll(thisProxyServerUrlHttps, actualUrlStr).replaceAll(thisProxyServerUrl_hostOnly, actualUrl.host)
		);
	}
	...
	if (response.status.toString().startsWith('3') && response.headers.get('Location') != null) {
		...
		try {
			...
			getHTMLResponse(
				redirectError + '<br>the redirect url:' + response.headers.get('Location') + ';the url you are now at:' + actualUrlStr
			);
		}
	}
